# 🔵 01 · Networking

> 10 hands-on labs covering enterprise networking concepts.

| Lab | Topics |
|-----|--------|
| 🔗 [Enterprise Network VLAN & InterVLAN Routing](./enterprise-network-vlan-intervlan-routing/) | VLAN segmentation, trunk ports, Layer 3 switching |
| 🔗 [Routing Protocols — OSPF, EIGRP, RIP, Static](./routing-protocols-ospf-eigrp-rip-static/) | Dynamic routing, administrative distance, route tables |
| 🌐 NAT & PAT Address Translation | Static NAT, dynamic NAT, PAT/overload |
| 📡 DHCP Multi-VLAN Deployment | DHCP pools, helper addresses, relay agents |
| 🧮 IP Addressing, Subnetting, VLSM & CIDR | Subnet calculations, VLSM design, CIDR notation |
| 🔧 Network Troubleshooting Case Studies | Systematic fault isolation, ping, traceroute, show commands |
| 🔄 HSRP — Network Redundancy & Failover | Active/standby, virtual IP, preemption |
| 🕸️ OSPF Virtual Link | Backbone connectivity, area 0 extension |
| 🗺️ OSPF Multi-Area — ABR & ASBR | Area border routers, route summarization |
| 🔀 Route Redistribution — OSPF & EIGRP | Inter-protocol routing, metric conversion |
